...
int main(int argc, char** argv)
{
    ...
    const int maxFontHeight = TTF_GetFontHeight(font);
    const int maxFontAscent = TTF_GetFontAscent(font);
    const int maxFontDescent = TTF_GetFontDescent(font);
    const int fontLineSkip = TTF_GetFontLineSkip(font);
    ...
    for(uint32_t i=0;i<NUM_GLYPHS;++i) {
        ...
        if(!gs[i]) {
            printf("TTF_RenderGlyph+Blended: %d : %s\n", glyph_id, SDL_GetError());
            gm[i].valid = false;
            continue;
        }

        TTF_GetGlyphMetrics(font, glyph_id,
                &gm[i].minx, &gm[i].maxx,
                &gm[i].miny, &gm[i].maxy,
                &gm[i].advance);
    ...
    SDL_Surface* fontTexture = SDL_CreateSurface(finalTextureWidth, finalTextureHeight,
            SDL_GetPixelFormatForMasks(32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000));

    const Uint32 black = SDL_MapSurfaceRGBA(fontTexture, 0,0,0, 0);
    //const Uint32 white = SDL_MapSurfaceRGBA(fontTexture->format, 255,255,255, 255);

    SDL_FillSurfaceRect(fontTexture, 0, black);
...
#if defined(USE_STBI)
    const SDL_PixelFormatDetails* pixDetails = SDL_GetPixelFormatDetails(fontTexture->format);
    stbi_write_tga(textureFile, fontTexture->w, fontTexture->h, pixDetails->bytes_per_pixel, fontTexture->pixels);
#else
    //IMG_SaveBMP(fontTexture, textureFile);
    // unfortunately, does not support RLE compression
    IMG_SaveTGA(fontTexture, textureFile);
#endif

...
void add_debug_mesh(struct RenderFrameContext *rfc, const RenderMesh *mesh, const mat4 &mat,
    ...
    rp->is_debug_pass = 1;
	rp->is_selection_pass = (selection_id && selection_id > ReservedObjIds::kGizmoLast) ? 1 : 0;
	rp->is_gizmo_pass = (selection_id && selection_id <= ReservedObjIds::kGizmoLast) ? 1 : 0;
...
void add_debug_mesh_constant_size_px(struct RenderFrameContext *rfc, const RenderMesh *mesh,
	...
	const float cam_z = rfc->b_is_perspective_ ? (rfc->view_ * tr_m.getTranslationPoint()).z : 1.0f;
	const float oo_w = rfc->proj_.elem[0][0];
    const float oo_h = rfc->proj_.elem[1][1];
    float width_by_2 = rfc->viewport_.z / 2.0f;
    float height_by_2 = rfc->viewport_.w / 2.0f;
...
void add_debug_mesh_constant_size_px(struct RenderFrameContext *rfc, const RenderMesh *mesh, float world_bounds,
	...
	const float oo_w = rfc->proj_.elem[0][0];
    //const float oo_h = rfc->proj_.elem[1][1];
    float width_by_2 = rfc->viewport_.z/2.0f;//800.0f / 2.0f;
    //float height_by_2 = rfc->viewport_.w/2.0f;//600.0f / 2.0f;
    float size = pixels * (cam_z * (1.0f/oo_w)) / width_by_2;
    //float size_y = pixels * (cam_z * (1.0f/oo_h)) / height_by_2;

	const mat4 tr = tr_m * mat4::scale((1.0f/world_bounds)*vec3(size));// * mat4::translation(vec3(-0.5f, 0.5f, 0));
	add_debug_mesh(rfc, mesh, tr, color, selection_id);
}

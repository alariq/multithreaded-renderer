...
struct CatmullRom {
    ...
    template <typename T>
    static float get_t(float t, float alpha, const T& p0, const T& p1 )
    {
        T d  = p1 - p0;
        float a = dot(d, d);
    ...
    {
        float t0 = 0.0f;
        float t1 = get_t<T>( t0, alpha, p0, p1 );
        float t2 = get_t<T>( t1, alpha, p1, p2 );
        float t3 = get_t<T>( t2, alpha, p2, p3 );
        t = lerp( t1, t2, t );
...
class Curve {
    ...
    public:
        ...
        T getAt(float t) const {
            ...
            if(segment < 0 || segment > pts_.size() - 4)
                return T(0);
        ...
        int getNSegments() const { return pts_.size() - 2; }
